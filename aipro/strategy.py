...
class MomentumStrategy:
    def decide(self, snapshot: MarketSnapshot) -> Decision:
        ...
        if snapshot.change_1h_pct >= 1.0:
            return Decision(snapshot.symbol, Signal.BUY, min(0.95, 0.55 + snapshot.change_1h_pct / 20), "positive momentum")
        if snapshot.change_1h_pct <= -1.0:
            return Decision(snapshot.symbol, Signal.SELL, min(0.95, 0.55 + abs(snapshot.change_1h_pct) / 20), "negative momentum")
