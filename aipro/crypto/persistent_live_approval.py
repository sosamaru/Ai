...
_STATES = frozenset({_REQUESTED, _CONFIRMED, _ACTIVE, _EXPIRED, _REVOKED})


@dataclass(frozen=True, slots=True)
class LiveApprovalState:
    """Persistent approval evidence for the guarded crypto command sequence."""
    ...
    def is_expired(self, *, now: datetime | None = None) -> bool:
        ...
        return reference.astimezone(UTC) >= datetime.fromisoformat(
            self.expires_at_utc
        ).astimezone(UTC)


class LiveApprovalError(RuntimeError):
    """Raised when a crypto LIVE approval transition fails closed."""


class LiveApprovalStore:
    """Persistent, crypto-only approval state with immutable audit events.

    This store records operator intent only. It never submits, cancels, replaces,
            ...
            connection.execute(
                ...
                CREATE TABLE IF NOT EXISTS crypto_live_approval (
                    ...
                    readiness_fingerprint TEXT NOT NULL
                        CHECK(length(readiness_fingerprint) = 64),
                    operator_fingerprint TEXT NOT NULL
                        CHECK(length(operator_fingerprint) = 64),
    ...
    def _read(self, connection: sqlite3.Connection) -> LiveApprovalState | None:
        ...
        state = LiveApprovalState(**dict(row))
        if state.state not in _STATES:
            raise LiveApprovalError("persisted approval state is invalid")
        self._validate_fingerprint(
            state.readiness_fingerprint,
            field_name="readiness_fingerprint",
        )
        self._validate_fingerprint(
            state.operator_fingerprint,
            field_name="operator_fingerprint",
        )
        try:
            timestamps = (
                datetime.fromisoformat(state.requested_at_utc),
                datetime.fromisoformat(state.expires_at_utc),
                None
                if state.confirmed_at_utc is None
                else datetime.fromisoformat(state.confirmed_at_utc),
                None
                if state.activated_at_utc is None
                else datetime.fromisoformat(state.activated_at_utc),
            )
        except ValueError as exc:
            raise LiveApprovalError("persisted approval timestamp is invalid") from exc
        if any(value is not None and value.tzinfo is None for value in timestamps):
            raise LiveApprovalError("persisted approval timestamp must be timezone-aware")
        return state
    ...
    def request(
        ...
        *,
        ...
        readiness = self._validate_fingerprint(
        ...
        with self._connect() as connection:
            ...
            if (
                ...
                and previous.state in {_REQUESTED, _CONFIRMED}
                and not previous.is_expired(now=reference)
            ):
    ...
    def activate(
        ...
        *,
        ...
        with self._connect() as connection:
            ...
            if state.operator_fingerprint != self._fingerprint_operator(operator_id):
                raise LiveApprovalError("operator mismatch")
            if not readiness_passed or readiness != state.readiness_fingerprint:
